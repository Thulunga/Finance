        ...
        {!showBudget && !showCash ? (
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
              <div className="rounded-lg border bg-emerald-500/5 p-2.5 sm:p-3"><p className="text-[11px] text-muted-foreground sm:text-xs">Budget utilization</p><p className="mt-1 font-mono text-lg font-semibold tabular-nums sm:text-xl">{kpis.percentage.toFixed(0)}%</p><p className="truncate text-[11px] text-muted-foreground sm:text-xs">{formatCurrency(kpis.totalSpent)} of {formatCurrency(kpis.totalAllocated)}</p></div>
              <div className="rounded-lg border bg-sky-500/5 p-2.5 sm:p-3"><p className="text-[11px] text-muted-foreground sm:text-xs">Income coverage</p><p className="mt-1 font-mono text-lg font-semibold tabular-nums sm:text-xl">{monthIncome > 0 ? ((kpis.totalSpent / monthIncome) * 100).toFixed(0) : "0"}%</p><p className="truncate text-[11px] text-muted-foreground sm:text-xs">Spending versus income</p></div>
              <div className="rounded-lg border bg-violet-500/5 p-2.5 sm:p-3"><p className="text-[11px] text-muted-foreground sm:text-xs">Credit card due</p><p className="mt-1 break-all font-mono text-lg font-semibold tabular-nums sm:text-xl">{formatCurrency(kpis.creditCardDue)}</p><p className="truncate text-[11px] text-muted-foreground sm:text-xs">Outstanding liability</p></div>
              <div className="rounded-lg border bg-amber-500/5 p-2.5 sm:p-3"><p className="text-[11px] text-muted-foreground sm:text-xs">Set aside</p><p className="mt-1 break-all font-mono text-lg font-semibold tabular-nums sm:text-xl">{formatCurrency(monthInvested + monthEmergency)}</p><p className="truncate text-[11px] text-muted-foreground sm:text-xs">Investment + reserve</p></div>
            </div>

            <div className="grid gap-3 xl:grid-cols-[1.35fr_0.65fr] xl:gap-5">
              <div className="min-w-0 rounded-lg border bg-background/60 p-2.5 sm:p-4">
